...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        scores = np.dot(X,W1) + np.tile(b1,(N,1))
        scores = np.fmax(np.zeros(scores.shape),scores)
        scores = np.dot(scores,W2) + np.tile(b2,(N,1))
		#the scores are correct even if no softmax dont know why
